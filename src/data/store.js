...
export const Store = {
    ...
    async getUnits() {
        const rawUnits = await DataAdapter.getUnits();
        return RBAC.filterUnits(rawUnits);
    },

    async getMembers() {
        const rawMembers = await DataAdapter.getMembers();
        return RBAC.filterMembers(rawMembers);
    },
    ...
    async saveScore(memberId, dateKey, scoreData) {
        ...
        // Record first log timestamp for this date (for admin 24h pending audit)
        // ONLY record if this is a real entry (present or absent) — never for pending/empty saves
        const isRealSave = scoreData.isAbsent === true ||
            (scoreData.isAbsent === false && scoreData.items && Object.keys(scoreData.items).length > 0);
        if (isRealSave) {
            DataAdapter.recordSessionLog(
                dateKey,
                fullScoreData.createdBy,
                fullScoreData.createdById
    ...
    // --- Session Logs (24h pending audit) ---
    async recordSessionLog(dateKey) {
        const currentUser = this.getCurrentUser();
        return DataAdapter.recordSessionLog(
            dateKey,
            currentUser?.name,
            currentUser?.id
        );
    ...
    async getSessionLogs() {
        return DataAdapter.getSessionLogs();
    },
